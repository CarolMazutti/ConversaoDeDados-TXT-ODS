...
def processamento(arquivo_txt):
    ...
    # Solicitar alíquota
    aliq_input = simpledialog.askstring(
        "Entrada de Dados", "Informe a alíquota (sem %):")
    ...
    tipo_servico = simpledialog.askstring(
        "Entrada de Dados", "Informe o Tipo de Serviço:")
    ...
    with open(arquivo_txt, 'r') as file:
        for linha in file:
            ...
            # Adiciona vírgula antes dos últimos dois caracteres
            valor_formatado = valor[:-2] + ',' + valor[-2:]
            base_formatado = base[:-2] + ',' + base[-2:]

            # Converter a data para o formato dd/mm/aaaa
            data_formatada = pd.to_datetime(
                data, format='%Y%m%d').strftime('%d/%m/%Y')

            # Adicionar os dados extraídos à lista
            dados.append([cpf_cnpj, data_formatada, nf,
                         valor_formatado, base_formatado])

        # Calcular ICMS e adicionar as novas colunas
        for i in range(len(dados)):
            # Remover a vírgula e converter BASE para float
            # Converter BASE para float
            base_valor = float(dados[i][4].replace(',', '.'))
            icms = base_valor * aliq / 100  # Calcular ICMS

            # Adicionar ALIQ, ICMS e TIPO DE SERVIÇO
            dados[i].extend(
                [aliq_formatado, f"{icms:.2f}".replace('.', ','), tipo_servico])

    # Criar um DataFrame com os dados
    df = pd.DataFrame(dados, columns=[
                      'CPF/CNPJ', 'DATA', 'NF', 'VALOR', 'BASE', 'ALIQ', 'ICMS', 'TIPO DE SERVIÇO'])
    ...
    botao_ods = tk.Button(formato_janela, text="Salvar como ODS", command=lambda: [
                          formato_janela.destroy(), salvar_arquivo('ods')])
    botao_ods.pack(pady=10)

    # Botão para salvar como CSV
    botao_csv = tk.Button(formato_janela, text="Salvar como CSV", command=lambda: [
                          formato_janela.destroy(), salvar_arquivo('csv')])
